...
def get_last_processed_id() -> int:
    ...
    try:
        ...
        if "Item" in response:
            ...
            metrics.add_metric(name="LastProcessedId", unit=MetricUnit.Count, value=last_id)
            return last_id
        else:
    ...
    try:
        ...
        for repo in repositories:
            flat_repo = {
                ...
                "license_key": repo.get("license", {}).get("key") if repo.get("license") else None,
            }
        ...
        metrics.add_metric(
            name="ParquetFileSize", unit=MetricUnit.Bytes, value=len(parquet_buffer.getvalue())
        )
...
def fetch_repositories(
    ...
    try:
        ...
        metrics.add_metric(
            name="RateLimitRemaining", unit=MetricUnit.Count, value=int(rate_limit_remaining or 0)
        )
        ...
        if response.status_code == 200:
            ...
            if retry_count < max_retries:
                ...
                return fetch_repositories(since_id, github_token, retry_count + 1, max_retries)
            return None
        elif response.status_code >= 500:
            logger.warning("GitHub API server error", extra={"status_code": response.status_code})
...
def crawl_repositories(
    ...
    for i in range(num_requests):
        ...
        if remaining_time < 30:
            ...
            metrics.add_metric(name="EarlyStopDueToTimeout", unit=MetricUnit.Count, value=1)
            break
        ...
        if repos is None:
            logger.warning("Failed to fetch repositories", extra={"current_id": current_id})
    ...
    metrics.add_metric(name="TotalRepositoriesFetched", unit=MetricUnit.Count, value=total_repos)
