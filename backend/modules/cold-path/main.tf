...
resource "aws_cloudwatch_metric_alarm" "user_crawler_errors" {
  ...
  dimensions = {
    FunctionName = aws_lambda_function.user_crawler.function_name
  }
...
resource "aws_cloudwatch_log_metric_filter" "user_crawler_progress" {
  ...
  log_group_name = aws_cloudwatch_log_group.user_crawler_lambda.name
  pattern        = "[time, request_id, level=INFO, msg=\"Crawl complete:\", ...]"
...
# Custom CloudWatch metric for users processed
