...
module "graphql_certificate" {
  count  = var.graphql_domain_name != "" && var.acm_certificate_arn == "" ? 1 : 0
  source = "../domain-certificate"
  ...
  domain_name      = var.graphql_domain_name
  hosted_zone_name = var.hosted_zone_name
...
module "graphql_dns" {
  count  = var.graphql_domain_name != "" ? 1 : 0
  source = "../domain-certificate"
  ...
  target_domain_name = aws_appsync_domain_name.dashboard[0].appsync_domain_name
  target_zone_id     = aws_appsync_domain_name.dashboard[0].hosted_zone_id

  enable_ipv6 = false # AppSync doesn't support IPv6
  tags        = var.tags

  depends_on = [aws_appsync_domain_name.dashboard]
}
